...
public static class AbortApplicationTaskJob
{
    ...
    public sealed class Handler : ICommandHandler<Command, Unit>
    {
        ...
        {
            var device = await _db.ApplicationTaskDevices
                .FirstOrDefaultAsync(d => d.ApplicationTaskId == cmd.TaskId && d.DeviceId == cmd.DeviceId, ct)
            ...
            await TaskReconciliation.ReconcileApplicationTaskAsync(_db, cmd.TaskId, ct);
    ...
    public static void Map(RouteGroupBuilder group) =>
        ...
        .RequireAuthorization(DeviceManagementPermissions.ApplicationTasks.Abort)
        .WithSummary("Skip a single device within a application task");
}
