...
/// <summary>
/// Lists product models eligible for configuration (Product.SupportsConfiguration = true).
/// Mirrors ListUpgradeableProducts.
/// </summary>
public static class ListConfigurableProducts
{
    ...
    public sealed class Handler : IRequestHandler<Query, List<ProductSelectDto>>
    {
        ...
        public async ValueTask<List<ProductSelectDto>> Handle(Query _, CancellationToken ct) =>
            await _db.Products
                .AsNoTracking()
                .Where(p => p.SupportsConfiguration && !p.IsDeleted)
