    ...
    where T : class
{
    ...
    /// <summary>
    /// Adds a string-based include path — the only way to express a multi-level ThenInclude
    /// chain (e.g. "Items.SoftwarePackageVersion.SoftwarePackage"), since <see cref="Include(Expression{Func{T,object}})"/>
    ...
    /// <summary>
    /// Enables <c>AsNoTracking()</c> for the query (default — already on unless disabled elsewhere).
    /// </summary>
    protected void AsNoTrackingQuery() => AsNoTracking = true;

    /// <summary>
    /// Enables <c>AsSplitQuery()</c> — use when the specification has multiple collection Includes.
    /// </summary>
    protected void AsSplitQueryEnabled() => AsSplitQuery = true;

    /// <summary>
    /// Enables ignoring EF Core global query filters (e.g. soft-delete) for this query.
    /// </summary>
